...
<head>
    ...
    <meta http-equiv="Content-Security-Policy" content="default-src * 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; script-src * 'self' 'unsafe-inline' 'unsafe-eval'; connect-src * 'self'; img-src * 'self' data: blob:; style-src * 'self' 'unsafe-inline';">
    ...
    <script crossorigin="anonymous" src="https://www.gstatic.com/firebasejs/10.12.2/firebase-analytics-compat.js"></script>
...
<body class="pb-24">
    ...
    <script>
        // ══════════════════════════════════════════
        // FIREBASE CONFIG — paste your web apiKey here
        // ══════════════════════════════════════════
        ...
        // ── Local cache helpers (fallback while Firebase loads) ──
        ...
        function saveAdminDB(data) {
            ...
            if (!fsMeta) {
                setTimeout(() => saveAdminDB(data), 1500);
                return;
            }
            const safeData = {
                tasks: data.tasks || [],
                usd_rate: data.usd_rate || 1500,
            ...
            fsMeta.set(safeData, { merge: true })
                .then(() => console.log('✅ AdminDB saved to Firestore'))
                .catch(e => {
                    console.error('❌ saveAdminDB failed:', e);
                    setTimeout(() => saveAdminDB(data), 2000);
                });
        }
        function getRate() {
            const db = loadAdminDB();
            return (db && db.usd_rate) ? db.usd_rate : 1500;
        }

        // ── userData ──
        let userData = {
            uid: "", name: "", phone: "", password: "",
            referral: "", country: "",
            wallet_balance: 0, redemption_balance: 0, usdt_balance: 0,
            history: [], loggedIn: false
        };

        // ── Save user to localStorage AND Firestore ──
        ...
        // ── Push userData to Firestore users/{uid} ──
        function syncUserToFirestore() {
            if (!userData.uid) return;
            if (!fsUsers) { setTimeout(syncUserToFirestore, 1500); return; }
            const payload = {
                id: userData.uid,
                uid: userData.uid,
                name: userData.name || '',
            ...
            fsUsers.doc(userData.uid).set(payload, { merge: true })
                .then(() => console.log('✅ User saved to Firestore:', userData.uid))
                .catch(e => {
                    console.error('❌ Firestore user write failed:', e);
                    // Retry once after 2 seconds
                    setTimeout(syncUserToFirestore, 2000);
                });
        }

        // ── Init Firebase and boot app ──
        window.addEventListener('load', () => {
            ...
            try {
                ...
                // Pull latest adminDB from Firestore into localStorage THEN render tasks
                ...
                }).catch(e => console.warn('Firestore meta read:', e));

                // Subscribe to real-time adminDB changes
            ...
            if (savedData) {
                try {
                    ...
                    if (userData.usdt_balance === undefined) userData.usdt_balance = 0;
                    if (userData.phone && userData.uid && userData.uid.startsWith('UID-')) userData.uid = userData.phone;
                } catch(e) { console.warn('userData parse error:', e); }
            }

            const swapAmountInput = document.getElementById('swap-amount');
            if (swapAmountInput) swapAmountInput.addEventListener('input', updateSwapPreview);

            setInterval(pollDashboardUpdates, 3000);

            // ── Splash timeout — ALWAYS runs regardless of Firebase ──
            setTimeout(() => {
                        ...
                        try {
                            ...
                            if (parsed && parsed.loggedIn) {
                                ...
                                if (leftAt && (Date.now() - parseInt(leftAt)) > INACTIVITY_MS) {
                                    ...
                                    // Try to refresh from Firestore, show app regardless
                                    if (fsUsers && parsed.uid) {
                                        fsUsers.doc(parsed.uid).get().then(snap => {
                                            if (snap && snap.exists) {
                                                const fData = snap.data();
                                                userData = { ...fData, loggedIn: true };
                                                localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
                                            }
                                            updateBalanceUI();
                                            updateHistoryUI();
        ...
        // ── Real-time listener on this user's Firestore doc (rewards, refunds) ──
        let userListener = null;
        function startFirestoreUserListener() {
            if (!fsUsers || !userData.uid) return;
            if (userListener) userListener(); // unsubscribe previous
            userListener = fsUsers.doc(userData.uid).onSnapshot(snap => {
                if (!snap || !snap.exists) return;
                const fData = snap.data();
                // Only pull if Firestore has a newer/higher balance (admin deposit/refund)
                const fHist = fData.history || [];
                const lHist = userData.history || [];
                if (fHist.length > lHist.length) {
                    const newEntries = fHist.slice(0, fHist.length - lHist.length);
                    const hasAdminEntry = newEntries.some(e => e.type === 'plus');
                    if (hasAdminEntry) {
                        userData.wallet_balance = fData.wallet_balance || 0;
                        userData.usdt_balance = fData.usdt_balance || 0;
                        userData.redemption_balance = fData.redemption_balance || 0;
                        userData.history = fHist;
                        localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
                        updateBalanceUI();
                        updateHistoryUI();
                        syncUserToAdminDB();
            ...
            }, e => console.warn('Firestore user snapshot:', e));
        }
        ...
        function signUp() {
            ...
            const doSignUp = () => {
                ...
                localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
                // Write to Firestore directly — retry built into syncUserToFirestore
                syncUserToFirestore();
                syncUserToAdminDB();
                startFirestoreUserListener();
                launchApp();
            ...
            if (fsUsers) {
                fsUsers.where('phone', '==', fullPhone).get().then(snap => {
                    if (!snap.empty) {
                    ...
                    doSignUp();
                }).catch(() => doSignUp()); // If Firestore unreachable, proceed anyway
            } else {
        ...
        function logIn() {
            ...
            const doLogin = (matchedUser) => {
                if (!matchedUser) { errEl.classList.remove('hidden'); return; }
                userData = {
                    uid: matchedUser.uid || matchedUser.phone,
                    name: matchedUser.name,
                    phone: matchedUser.phone,
                    password: matchedUser.password,
                    referral: matchedUser.referral || '',
                    country: matchedUser.country || 'Nigeria',
                    wallet_balance: matchedUser.wallet_balance || 0,
                    usdt_balance: matchedUser.usdt_balance || 0,
                    redemption_balance: matchedUser.redemption_balance || 0,
            ...
            const tryLogin = () => {
                if (!fsUsers) { errEl.classList.remove('hidden'); return; }
                // Query Firestore directly — works across all devices
                fsUsers.get().then(allSnap => {
                    const match = allSnap.docs.find(d => {
                        const u = d.data();
                        return u.name && u.name.toLowerCase() === name.toLowerCase() && u.password === password;
                    });
                    if (match) {
                        doLogin(match.data());
                    } else {
                        // Fallback: check local storage (same device only)
                    ...
                    // Firestore error — try local only
                    const local = localStorage.getItem('capacity_hub_core_data');
                    if (local) {
                        const p = JSON.parse(local);
                        if (p.name && p.name.toLowerCase() === name.toLowerCase() && p.password === password) {
        ...
        // ── Register / sync this user into the shared admin DB ──
        function syncUserToAdminDB() {
            const db = loadAdminDB();
            if (!db) return;
            if (!db.users) db.users = [];
            const existing = db.users.find(u => u.uid === userData.uid);
            if (!existing) {
                // First time — register user
                db.users.push({
                    id: userData.uid,
                    uid: userData.uid,
                    name: userData.name || 'App User',
                    phone: userData.phone || '—',
                    password: userData.password || '—',
                ...
                // Always sync local balances to dashboard.
                // The poll (pollDashboardUpdates) handles pulling admin-approved
                // increases back into local — so whatever is in userData IS correct.
                existing.wallet_balance = userData.wallet_balance;
                existing.usdt_balance = userData.usdt_balance;
                existing.redemption_balance = userData.redemption_balance;
                // Only push history if local has entries (don't wipe admin-added entries)
                if ((userData.history || []).length >= (existing.history || []).length) {
                    existing.history = userData.history;
                }
                // Always sync profile fields
                existing.name = userData.name || existing.name;
                existing.phone = userData.phone || existing.phone;
                existing.password = userData.password || existing.password;
                existing.country = userData.country || existing.country;
            }
            saveAdminDB(db);
        }

        // ── Poll dashboard DB for balance/history changes pushed by admin ──
        ...
        function pollDashboardUpdates() {
            ...
            // New task published — show notification
            const taskCount = (db.tasks || []).filter(t => t.status === 'active').length;
            if (lastTaskCount === -1) {
                lastTaskCount = taskCount; // initialise silently on first poll
            } else if (taskCount > lastTaskCount) {
                lastTaskCount = taskCount;
                renderTasksFromDashboard();
                // Push new task notification into history
                const newest = [...(db.tasks || [])].filter(t => t.status === 'active').pop();
                if (newest) {
                    const notifEntry = {
                        title: `New Task: ${newest.title}`,
                        amount: newest.reward,
                        type: 'notif',
                        date: new Date().toLocaleString('en-NG', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' })
                    };
                    userData.history.unshift(notifEntry);
                    saveToPhone();
                    updateHistoryUI();
                    document.getElementById('notif-dot').classList.remove('hidden');
                }
            ...
            // Rate changed — refresh balance display
            ...
            // Only pull from dashboard if admin added a NEW history entry
            // (task approval or withdrawal refund) — identified by entry type 'plus'
            // that doesn't exist in local history yet
            const dbHist = dbUser.history || [];
            const localHist = userData.history || [];
            if (lastHistoryLength === 0) lastHistoryLength = localHist.length;

            if (dbHist.length > lastHistoryLength) {
                // Check the new entries are admin-originated (plus type = deposit/refund)
                const newEntries = dbHist.slice(0, dbHist.length - lastHistoryLength);
                const hasAdminEntry = newEntries.some(e => e.type === 'plus');
                if (hasAdminEntry) {
        ...
        function processWithdrawal() {
            ...
            if (targetField === 'usdt_balance' || targetField === 'wallet_balance') {
                ...
                setTimeout(() => {
                    ...
                    const isUSDT = targetField === 'usdt_balance';
