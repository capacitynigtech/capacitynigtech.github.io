...
<body class="pb-24">
    ...
    <script>
        ...
        const TG_BOT_B = null; // Tasks bot — to be added
        const TG_BOT_C = null; // Withdrawals bot — to be added

        function tgSend(botToken, chatId, text) {
            try {
                // Hidden iframe + form POST — bypasses all CORS/CSP restrictions.
                // The browser posts the form into the iframe (a normal navigation),
                // which is never blocked by CORS or Content-Security-Policy.
                const iframeId = 'tg_iframe_' + Math.random().toString(36).substr(2, 6);
                ...
                const fields = { chat_id: chatId, text: text, parse_mode: 'HTML' };
        ...
        async function submitTaskProof(taskId) {
            ...
            if (TG_BOT_B) tgSend(TG_BOT_B, TG_ADMIN_CHAT,
                '📋 <b>Task Submission</b>\n' +
        ...
        async function processWithdrawal() {
            ...
            if (targetField === 'usdt_balance' || targetField === 'wallet_balance') {
                ...
                setTimeout(async () => {
                    ...
                    if (TG_BOT_C) tgSend(TG_BOT_C, TG_ADMIN_CHAT,
