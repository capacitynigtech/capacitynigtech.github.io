...
<head>
    ...
    <script>
        // Suppress Tailwind production warning before it loads
        Object.defineProperty(window, '__NEXT_DATA__', { value: {} });
...
<body class="pb-24">
    ...
    <!-- Firebase SDK — loaded here so DOM is ready first -->
    ...
    <script>
        ...
        // ── Firebase refs ──
        let db_fire = null, fsUsers = null, fsMeta = null, userListener = null;
        ...
        // Queue for pending writes when Firebase not ready yet
        let _writeQueue = [];
        function _flushQueue() {
            while (_writeQueue.length) {
                const fn = _writeQueue.shift();
                try { fn(); } catch(e) { console.error('Queue flush error:', e); }
            }
        }

        // Write user to Firestore — queues if not ready
        function syncUserToFirestore() {
            if (!userData.uid) return;
            const payload = {
                id: userData.uid, uid: userData.uid,
                name: userData.name || '',
            ...
            const doWrite = () => {
                fsUsers.doc(userData.uid).set(payload, { merge: true })
                    .then(() => console.log('✅ User synced to Firestore:', userData.uid))
                    .catch(e => console.error('❌ User write error:', e));
            };
            if (fsUsers) { doWrite(); }
            else { _writeQueue.push(doWrite); }
        }

        // Save app state to localStorage + Firestore
        function saveToPhone() {
            localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
            syncUserToFirestore();
        }

        // Write meta doc (submissions, withdrawals) to Firestore
        function saveAdminDB(data) {
            ...
            const safeData = {
                ...
                submissions: (data.submissions || []).map(s => ({
                    ...
                    screenshot: (s.screenshot||'').startsWith('data:') ? '[image_uploaded]' : (s.screenshot||''),
            ...
            const doWrite = () => {
                fsMeta.set(safeData, { merge: true })
                    .then(() => console.log('✅ AdminDB synced'))
                    .catch(e => console.error('❌ AdminDB write error:', e));
            };
            if (fsMeta) { doWrite(); }
            else { _writeQueue.push(doWrite); }
        }
        ...
        // ══════════════════════════════════════════
        // FIREBASE INIT — retries until SDK ready
        // ══════════════════════════════════════════
        function initFirebase() {
            ...
            try {
                ...
                // Flush any writes that were queued before Firebase was ready
                _flushQueue();

                // Listen to meta doc — tasks, rate, submissions flow down to app
                fsMeta.onSnapshot(snap => {
                    if (snap && snap.exists) {
                        const data = snap.data();
                        // Merge into localStorage cache
                        const local = loadAdminDB() || {};
                        const merged = { ...local, ...data };
                        localStorage.setItem('ch_admin_db', JSON.stringify(merged));
        ...
        // ══════════════════════════════════════════
        // SPLASH BOOT — 100% independent of Firebase
        // ══════════════════════════════════════════
        function bootSplash() {
            // Restore saved user
            try {
            ...
            setInterval(pollDashboardUpdates, 4000);

            // Splash hides after exactly 3s — NOTHING can block this
            setTimeout(() => {
        ...
        function showNextScreen() {
            try {
                ...
                if (parsed && parsed.loggedIn) {
                    ...
                    const tryRefresh = () => {
                        if (!fsUsers) { setTimeout(tryRefresh, 500); return; }
                        fsUsers.doc(parsed.uid).get()
                            .then(snap => {
                                if (snap && snap.exists) {
                                    userData = { ...snap.data(), loggedIn: true };
                                    localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
                                    updateBalanceUI();
                                    updateHistoryUI();
                                }
                                startFirestoreUserListener();
                            }).catch(() => startFirestoreUserListener());
                    };
                    tryRefresh();
                } else {
                    document.getElementById('auth-screen').classList.remove('hidden');
                ...
                console.error('showNextScreen:', e);
                document.getElementById('auth-screen').classList.remove('hidden');
            }
        }

        // Start Firebase and splash in parallel
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => { initFirebase(); bootSplash(); });
        } else {
            initFirebase();
            bootSplash();
        }

        // ── Real-time listener — detects admin reward deposits and refunds ──
        function startFirestoreUserListener() {
            if (!userData.uid) return;
            if (!fsUsers) { setTimeout(startFirestoreUserListener, 500); return; }
            ...
            userListener = fsUsers.doc(userData.uid).onSnapshot(snap => {
                ...
                // If Firestore has more history entries than local — admin added something
                if (fHist.length > lHist.length) {
                    ...
                    console.log('✅ Admin reward received from Firestore');
                }
            }, e => console.error('❌ User snapshot error:', e));
        }
        ...
        function launchApp() {
            ...
            updateHistoryUI();
            renderTasksFromDashboard();
            syncUserToFirestore();       // ensures user is in Firestore
            startFirestoreUserListener(); // listen for admin rewards
            startInactivityWatcher();
        }

        // ── Sync user to admin DB (Firestore only — localStorage is just a local cache) ──
        function syncUserToAdminDB() {
            syncUserToFirestore();
        }

        // ── Poll local cache for task/rate changes from dashboard ──
        let lastTaskCount = -1;
        let lastRate = 0;
