...
<head>
    ...
    <!-- FALLBACK SYSTEM: Tries Internet first, then local files for Zero Kobo support -->
    ...
    <style>
        ...
        body { font-family: 'Inter', sans-serif; overflow: hidden; touch-action: manipulation; background-color: #f3f4f6; margin: 0; padding: 0; }
        ...
        button, .cursor-pointer { -webkit-tap-highlight-color: transparent; outline: none; border: none; background: none; }
...
<body class="flex justify-center min-h-screen">
    ...
    <div id="main-app" class="w-full max-w-[450px] bg-white shadow-2xl min-h-screen flex flex-col relative overflow-hidden">
        ...
        <!-- EXIT PROMPT -->
        <div id="exit-modal" class="absolute inset-0 z-[110] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm hidden">
            <div class="bg-white w-full rounded-2xl overflow-hidden shadow-2xl animate-scale-up">
                <div class="bg-gray-800 p-4 flex justify-center"><i data-lucide="log-out" class="text-white w-12 h-12"></i></div>
                <div class="p-6 text-center">
                    <h3 class="text-lg font-black text-gray-800 uppercase tracking-tight mb-2">Exit App</h3>
                    <p class="text-gray-500 font-bold text-sm mb-6">Do you want to exit Capacity VPN?</p>
                    <div class="flex gap-3">
                        <button onclick="hideExitModal()" class="flex-1 py-3 border-2 border-gray-100 rounded-xl font-black text-xs uppercase text-gray-400 active:bg-gray-50">No</button>
                        <button onclick="confirmExit()" class="flex-1 py-3 bg-gray-800 rounded-xl font-black text-xs uppercase text-white shadow-lg active:bg-gray-900">Exit</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        window.onload = () => {
            ...
            } catch(e) { console.error("Icon load fail", e); }
        ...
        async function fetchNodes() {
            ...
            try {
                ...
                showAlert("Update failed. Check data.");
            } finally {
        ...
        async function executeNativeVpn() {
            ...
            await sleep(1500); // Simulate Handshake
        ...
        // FUNCTIONAL: SHARE APP
        async function shareApp() {
            const shareData = {
                title: 'Capacity VPN',
                text: 'Download Capacity VPN for free internet access! fast and reliable.',
        ...
        function confirmExit() {
            if (connected) confirmDisconnect();
            saveToPhoneStorage();
            
            // UI Feedback
            document.body.innerHTML = `
                <div class="flex flex-col items-center justify-center h-screen bg-gray-900 text-white p-10 text-center">
                    <div class="w-20 h-20 bg-orange-500 rounded-full flex items-center justify-center mb-6">
                        <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                    </div>
                    <h1 class="text-2xl font-black mb-2 uppercase italic">Capacity VPN</h1>
                    <p class="text-gray-400 font-bold mb-8">Service safely closed.</p>
                    <button onclick="location.reload()" class="px-8 py-3 bg-orange-500 rounded-xl font-black uppercase text-xs">Re-Open App</button>
                </div>
            `;
