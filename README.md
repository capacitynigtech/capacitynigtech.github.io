...
<body class="pb-24">
    ...
    <script>
        ...
        function saveAdminDB(data) {
            ...
            // Also push to Firestore meta doc (tasks, withdrawals, submissions, rate)
            if (fsMeta) {
                fsMeta.set(data, { merge: true }).catch(e => console.warn('Firestore meta write:', e));
            }
        }
        function getRate() {
        ...
        function syncUserToFirestore() {
            ...
            }, { merge: true }).catch(e => console.warn('Firestore user write:', e));
        }
        ...
        window.addEventListener('load', () => {
            ...
            try {
                ...
                // Pull latest adminDB from Firestore into localStorage
                    ...
                    setTimeout(() => {
                        ...
                        try {
                            ...
                            if (parsed && parsed.loggedIn) {
                                // Try to refresh from Firestore, but show app regardless
                            ...
                            // Safety net — always show auth if anything fails
        ...
        function signUp() {
            ...
            if (fsUsers) {
                fsUsers.where('phone', '==', fullPhone).get().then(snap => {
                    ...
                    // Firestore unavailable — fall back to local check
                    const db = loadAdminDB();
                    if (db && db.users && db.users.find(u => u.phone === fullPhone)) {
                        errEl.textContent = 'This phone number is already registered.';
                        errEl.classList.remove('hidden');
                        return;
                    }
        ...
        function logIn() {
            ...
            // Try Firestore first
            if (fsUsers) {
                fsUsers.where('name', '==', name).where('password', '==', password).get()
                .then(snap => {
                    if (!snap.empty) {
                        doLogin(snap.docs[0].data());
                    } else {
                        // Try case-insensitive fallback via all users
                        fsUsers.get().then(allSnap => {
                            const match = allSnap.docs.find(d => {
                                const u = d.data();
                                return u.name && u.name.toLowerCase() === name.toLowerCase() && u.password === password;
                            });
                            if (match) { doLogin(match.data()); }
                            else {
                                // Fallback to local
                                const local = localStorage.getItem('capacity_hub_core_data');
                                if (local) {
                                    const parsed = JSON.parse(local);
                                    if (parsed.name && parsed.name.toLowerCase() === name.toLowerCase() && parsed.password === password) {
                                        doLogin(parsed); return;
                                    }
                                }
                                errEl.classList.remove('hidden');
                            }
                    ...
                    // Firestore unavailable — check local
                    const db = loadAdminDB();
                    let m = null;
                    if (db && db.users) m = db.users.find(u => u.name && u.name.toLowerCase() === name.toLowerCase() && u.password === password);
                    if (!m) {
                        const local = localStorage.getItem('capacity_hub_core_data');
                        if (local) { const p = JSON.parse(local); if (p.name && p.name.toLowerCase() === name.toLowerCase() && p.password === password) m = p; }
                    }
                    doLogin(m);
                });
            } else {
