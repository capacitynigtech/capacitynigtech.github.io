...
<body class="pb-24">
    ...
    <script>
        ...
        const JB_BIN_ID     = '6a1436c9c397f822d0d0db7f';   // shared data bin (users/tasks/withdrawals)
        const JB_BASE       = 'https://api.jsonbin.io/v3/b';

        // ── JSONBin helpers ──────────────────────
        async function jbRead() {
            try {
                const r = await fetch(`${JB_BASE}/${JB_BIN_ID}/latest`, {
        ...
        async function jbWrite(data) {
            try {
                const r = await fetch(`${JB_BASE}/${JB_BIN_ID}`, {
                    method: 'PUT',
                    headers: {
        ...
        async function jbPatch(fn) {
            let record = await jbRead();
            if (!record) record = { users: [], tasks: [], withdrawals: [] };
            if (!record.users) record.users = [];
            if (!record.tasks) record.tasks = [];
            if (!record.withdrawals) record.withdrawals = [];
            fn(record);
            return await jbWrite(record);
        }

        // ══════════════════════════════════════════
        // BACKEND SHIM  (same API surface as Supabase helpers)
        // ══════════════════════════════════════════

        // Upsert a user into the bin
        async function sbSaveUser() {
            ...
            try {
                await jbPatch(rec => {
                    ...
                    const idx = rec.users.findIndex(u => u.uid === p.uid);
                    if (idx >= 0) rec.users[idx] = { ...rec.users[idx], ...p };
                    else rec.users.unshift(p);
                });
                console.log('✅ User saved to jsonbin:', userData.uid);
            } catch(e) { console.error('❌ sbSaveUser:', e.message); }
        }

        async function sbLoadUser(uid) {
            try {
                const rec = await jbRead();
                if (!rec || !rec.users) return null;
                return rec.users.find(u => u.uid === uid) || null;
            } catch(e) { return null; }
        }

        async function sbPhoneExists(phone) {
            try {
                const rec = await jbRead();
                if (!rec || !rec.users) return false;
                return rec.users.some(u => u.phone === phone);
            } catch(e) { return false; }
        }
        ...
        async function sbLogin(name, password) {
            try {
                const rec = await jbRead();
                if (!rec || !rec.users) return null;
                return rec.users.find(
                    u => u.name && u.name.toLowerCase() === name.toLowerCase() && u.password === password
                ) || null;
            } catch(e) { return null; }
        }

        async function sbLoadAppData() {
            try {
                const rec = await jbRead();
                if (!rec) return;
                // Merge tasks/rate/submissions into local admin db
                const local = loadAdminDB() || {};
                if (rec.tasks && rec.tasks.length) local.tasks = rec.tasks;
                if (rec.withdrawals) local.withdrawals = rec.withdrawals;
                if (rec.submissions) local.submissions = rec.submissions;
                if (rec.usd_rate) local.usd_rate = rec.usd_rate;
                if (rec.app_meta) Object.assign(local, rec.app_meta);
                saveAdminDB(local);
            } catch(e) { console.error('❌ sbLoadAppData:', e.message); }
        }
        ...
        async function sbSaveSubmission(sub) {
            try {
                await jbPatch(rec => {
                    ...
                    const idx = rec.submissions.findIndex(s => s.id === sub.id);
                    if (idx >= 0) rec.submissions[idx] = sub;
                    else rec.submissions.unshift(sub);
                });
                console.log('✅ Submission saved to jsonbin');
            } catch(e) { console.error('❌ sbSaveSubmission:', e.message); }
        }

        async function sbSaveWithdrawal(wd) {
            try {
                await jbPatch(rec => {
                    if (!rec.withdrawals) rec.withdrawals = [];
                    const idx = rec.withdrawals.findIndex(w => w.id === wd.id);
                    if (idx >= 0) rec.withdrawals[idx] = wd;
                    else rec.withdrawals.unshift(wd);
                });
                console.log('✅ Withdrawal saved to jsonbin');
            } catch(e) { console.error('❌ sbSaveWithdrawal:', e.message); }
        }

        // Real-time polling — replaces Supabase realtime channel
        ...
        function startUserListener() {
            ...
            _pollInterval = setInterval(async () => {
                ...
                // Sync app meta (tasks, rate, etc.)
                const local = loadAdminDB() || {};
                let changed = false;
                if (rec.tasks) { local.tasks = rec.tasks; changed = true; }
                if (rec.withdrawals) { local.withdrawals = rec.withdrawals; changed = true; }
                if (rec.submissions) { local.submissions = rec.submissions; changed = true; }
                if (rec.usd_rate) { local.usd_rate = rec.usd_rate; changed = true; }
                if (rec.app_meta) { Object.assign(local, rec.app_meta); changed = true; }
                if (changed) {
                    saveAdminDB(local);
                    renderTasksFromDashboard();
                }

                // Sync user balances from server
                const fresh = rec.users && rec.users.find(u => u.uid === userData.uid);
                if (fresh) {
                    const balanceChanged =
                        fresh.wallet_balance !== userData.wallet_balance ||
                        fresh.usdt_balance !== userData.usdt_balance ||
                        (fresh.history||[]).length > (userData.history||[]).length;
                    if (balanceChanged) {
                        userData.wallet_balance = fresh.wallet_balance || 0;
                        userData.usdt_balance = fresh.usdt_balance || 0;
                        userData.redemption_balance = fresh.redemption_balance || 0;
                        userData.history = fresh.history || [];
                        localStorage.setItem('capacity_hub_core_data', JSON.stringify({...userData, loggedIn:true}));
                        updateBalanceUI();
                        updateHistoryUI();
                        const dot = document.getElementById('notif-dot');
                        if (dot) dot.classList.remove('hidden');
                        console.log('✅ Balance synced from jsonbin');
                    }
                }
        ...
        function startAppMetaListener() {
            // Handled inside startUserListener polling loop
        }

        // Init jsonbin (replaces initSupabase)
        function initSupabase() {
            console.log('✅ JSONBin.io backend ready');
            sbLoadAppData();
            if (userData && userData.uid) startUserListener();
        }

        // ══════════════════════════════════════════
        // TELEGRAM CONFIG (for server.js reference)
        // ══════════════════════════════════════════
        ...
        function showNextScreen() {
            try {
                ...
                if (parsed && parsed.loggedIn) {
                    ...
                    // Refresh from jsonbin in background — retries until ready
                    const doRefresh = () => {
                        sbLoadUser(userData.uid).then(fresh => {
                            if (fresh) {
        ...
        async function signUp() {
            ...
            const exists = await sbPhoneExists(fullPhone);
            if (exists) {
        ...
        async function logIn() {
            ...
            // Try jsonbin first
            let match = await sbLogin(name, password);
