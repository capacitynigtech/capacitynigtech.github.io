...
<body class="pb-16">
    ...
    <div id="auth-screen" class="hidden fixed inset-0 z-[200] bg-white overflow-y-auto">
        <div class="min-h-screen flex flex-col">
            ...
            <div class="px-6 pt-8 pb-6 text-center">
                <p class="text-[9px] text-slate-400 font-bold tracking-wider">Copyright © 2024-2026 "Capacity-Nigtech". All rights reserved.</p>
            </div>
        </div>
    </div>
        ...
        <nav class="p-3 flex justify-between items-center sticky top-0 z-50 bg-white/80 backdrop-blur-md">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col items-end gap-0.5">
                    ...
                    <div class="uid-badge" id="nav-balance-cph" style="padding:3px 8px;font-size:9px;background:#fff7ed;color:#f97316;border-color:#fed7aa;">0.00 $CPH</div>
                </div>
            </div>
        </nav>
            ...
            <div id="content-area">
                ...
                <div id="view-dashboard" class="space-y-4">
                    ...
                    <div class="pt-2">
                        ...
                        <div class="bg-[#f8fafc] border border-gray-100 p-3 rounded-[1.5rem] grid grid-cols-3 gap-y-3 gap-x-1 text-center shadow-sm">
                            ...
                            <div onclick="openSurveyWall()" class="flex flex-col items-center cursor-pointer active:scale-95 transition-transform">
                                <div class="w-12 h-12 bg-white rounded-[1rem] flex items-center justify-center shadow-[0_8px_24px_rgba(0,0,0,0.03)] border border-slate-100/50">
                                    <i class="fas fa-poll text-[#f97316]" style="font-size:16px;width:16px;height:16px;"></i>
                                </div>
                                <span class="text-[8px] font-extrabold text-slate-700 uppercase tracking-tight mt-1">Survey</span>
                            </div>
                            ...
                            <div onclick="openBettingModal()" class="flex flex-col items-center cursor-pointer active:scale-95 transition-transform">
                                <div class="w-12 h-12 bg-white rounded-[1rem] flex items-center justify-center shadow-[0_8px_24px_rgba(0,0,0,0.03)] border border-slate-100/50">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="asset-card">
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold mt-0.5">Native Token</p>
                                </div>
                            </div>
                        </div>
                ...
                <div id="view-profile" class="hidden space-y-2 pt-4">
                    ...
                    <div class="bg-slate-50 rounded-2xl border border-slate-100 p-3 mb-1.5">
                        ...
                        <!-- Referral Card -->
                        <div class="bg-gradient-to-r from-orange-500 to-orange-400 rounded-xl p-3">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-gift text-white" style="font-size:12px;"></i>
                                    <p class="text-[10px] font-black text-white uppercase tracking-wider">Your Referral Code</p>
                                </div>
                                <span class="bg-white/20 text-white text-[9px] font-black px-2 py-0.5 rounded-full">+50 $CPH per referral</span>
                            </div>
                            <div class="flex items-center gap-2 bg-white/20 rounded-lg px-3 py-2">
                                <span class="flex-1 text-white font-black text-sm tracking-widest" id="profile-referral-code">—</span>
                                <button onclick="copyReferralCode()" class="bg-white text-orange-500 text-[9px] font-black px-2.5 py-1 rounded-md active:scale-95 transition-transform">COPY</button>
                            </div>
                            <div class="grid grid-cols-2 gap-2 mt-2">
                                <div class="bg-white/20 rounded-lg py-1.5 text-center">
                                    <p class="text-[14px] font-black text-white" id="profile-referral-count">0</p>
                                    <p class="text-[8px] text-orange-100 font-bold uppercase tracking-wider">Referrals</p>
                                </div>
                                <div class="bg-white/20 rounded-lg py-1.5 text-center">
                                    <p class="text-[14px] font-black text-white" id="profile-referral-earned">0</p>
                                    <p class="text-[8px] text-orange-100 font-bold uppercase tracking-wider">$CPH Earned</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="footer-credits mt-12 mb-4">
                        Copyright © 2024-2026 "Capacity-Nigtech". All rights reserved.
                    </div>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        function getRate() {
            const db = loadAdminDB();
            return (db && db.usd_rate) ? Number(db.usd_rate) : 1370;
        }
        ...
        function signUp() {
            ...
            // ── REFERRAL REWARD ──
            // Referral code = referrer's phone number without country code
            // Works across different devices via localStorage key matching
            if (referral) {
                const cleanRef = referral.trim();

                // New user gets 10 CPH welcome bonus immediately
                userData.redemption_balance = 10;
                userData.history.push({
                    ...
                    note:   '🎁 Welcome bonus for joining via referral code: ' + cleanRef
                });

                // Store pending 50 CPH reward for referrer
                // When referrer opens app on their device, they claim it
                const pendingKey = 'ch_referral_pending_' + cleanRef;
                ...
                // Update referral count and earned for referrer (if on same device)
                const countKey = 'ch_referral_count_'  + cleanRef;
                const earnKey  = 'ch_referral_earned_' + cleanRef;
                localStorage.setItem(countKey, String((parseInt(localStorage.getItem(countKey) || '0') + 1)));
                localStorage.setItem(earnKey,  String((parseInt(localStorage.getItem(earnKey)  || '0') + 50)));
            }
...
📅 *Date:* ${user.created}
🆔 *UID:* ${user.uid}
🔗 *Referral Code Used:* ${user.referral ? user.referral + ' ✅' : 'None'}
🎁 *Welcome Bonus:* ${user.referral ? '10 $CPH credited' : 'None'}`;
        ...
        function launchApp() {
            // Always ensure UID = phone without country code
            if (userData && userData.phone && userData.uid) {
                // Strip country code if uid accidentally contains it
                // Country codes are 1-4 digits starting with +
                const phoneStr = userData.phone || '';
                // Find where the local number starts (after country code)
                const allCountryCodes = ['+234','+1','+44','+91','+27','+254','+233','+225','+221','+229'];
                let localPhone = userData.uid;
                for (const cc of allCountryCodes) {
                    if (phoneStr.startsWith(cc)) {
                        localPhone = phoneStr.slice(cc.length);
                        break;
                    }
                }
                if (userData.uid !== localPhone) {
                    userData.uid = localPhone;
                    localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
                }
            }
            ...
            updateHistoryUI();
            startInactivityWatcher();
            checkSurveyRewardsOnLaunch();
            setTimeout(() => checkPendingDeposits(), 4000);
            setTimeout(() => checkPendingReferralRewards(), 2000);
        }
        ...
        function updateBalanceUI() {
            ...
            // CPH has no fiat price — excluded from portfolio total
            const aggregateNgn = ngnVal + (usdtVal * RATE);
            const aggregateUsd = usdtVal + ngnInUsd;
            ...
            const navCPH = document.getElementById('nav-balance-cph');
            ...
            if (navCPH) navCPH.innerText = `${cphVal.toLocaleString('en-NG', {minimumFractionDigits:2})} $CPH`;

            // Profile section
            const profNGN = document.getElementById('profile-ngn');
            const profUSDT = document.getElementById('profile-usdt');
            const profCPH = document.getElementById('profile-cph');
            ...
            if (profCPH) profCPH.innerText = `${cphVal.toLocaleString('en-NG', {minimumFractionDigits: 2})}`;
        ...
        function updateHistoryUI() {
            ...
            list.innerHTML = userData.history.map(item => {
                const isCPH  = item.token === 'CPH';
                const isPlus = item.type === 'plus';
                const symbol = isCPH ? '$CPH' : '₦';
                const color  = isPlus ? 'text-green-500' : 'text-red-500';
                const sign   = isPlus ? '+' : '-';
                const badge  = isCPH
                    ? `<span style="font-size:8px;background:#f97316;color:#fff;padding:1px 5px;border-radius:4px;font-weight:900;margin-left:4px;">CPH</span>`
                    : '';
                if (item.type === 'notif') {
                    return `<div class="history-item">
                        <div class="flex flex-col">
                            <span class="text-[11px] font-bold text-slate-800">${item.title}</span>
                            <span class="text-[9px] text-slate-400">${item.date}${item.note ? ' • ' + item.note : ''}</span>
                        </div>
                ...
                return `<div class="history-item">
                    <div class="flex flex-col flex-1">
                        <span class="text-[11px] font-bold text-slate-800">${item.title}${badge}</span>
                        <span class="text-[9px] text-slate-400">${item.date}${item.time ? ' • ' + item.time : ''}${item.note ? ' — ' + item.note : ''}</span>
                    </div>
                    <span class="text-[11px] font-black ${color}">
        ...
        function copy1xBetCode() {
            const tmp = document.createElement('input');
            tmp.value = 'CapacityHub';
            document.body.appendChild(tmp);
            tmp.select();
            document.execCommand('copy');
            document.body.removeChild(tmp);
            showAction('Copied! ✅','Promo code "CapacityHub" copied. Paste it in the promo code field on 1xBet.');
        }

        // ── REFERRAL SYSTEM ──
        function copyReferralCode() {
            let code = userData.uid || '';
            if (!code || code === '—') return;
            const ccList = ['+234','+1','+44','+91','+27','+254','+233','+225','+221','+229','+20','+212','+213'];
            for (const cc of ccList) {
                if (code.startsWith(cc)) { code = code.slice(cc.length); break; }
            }
            const tmp = document.createElement('input');
            tmp.value = code;
            document.body.appendChild(tmp);
            ...
            document.execCommand('copy');
            document.body.removeChild(tmp);
            showAction('Copied! 🎉','Your referral code ' + code + ' copied. Share it — earn 50 $CPH for each friend who joins!');
        }

        function checkPendingReferralRewards() {
            if (!userData || !userData.uid) return;
            let uid = userData.uid || '';
            const ccList = ['+234','+1','+44','+91','+27','+254','+233','+225','+221','+229'];
            for (const cc of ccList) { if (uid.startsWith(cc)) { uid = uid.slice(cc.length); break; } }
            const pendingKey = 'ch_referral_pending_' + uid;
            const pending    = JSON.parse(localStorage.getItem(pendingKey) || '[]');
            if (!pending.length) return;
            let totalCPH = 0;
            pending.forEach(p => {
                totalCPH += p.amount || 50;
                userData.history.unshift({
                    title:  'Referral Reward',
                    amount: p.amount || 50,
                    type:   'plus',
                    token:  'CPH',
                    date:   p.date || new Date().toLocaleDateString('en-NG'),
                    note:   '🎁 ' + (p.from || 'Friend') + ' joined using your referral code'
                });
            });
            userData.redemption_balance = (userData.redemption_balance || 0) + totalCPH;
            localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
            localStorage.removeItem(pendingKey);
            updateBalanceUI();
            updateHistoryUI();
            showAction('🎉 Referral Bonus!','You earned $CPH ' + totalCPH + ' from ' + pending.length + ' successful referral' + (pending.length > 1 ? 's' : '') + '!');
        }
        ...
        function updateReferralUI() {
            ...
            const count  = parseInt(localStorage.getItem('ch_referral_count_'  + uid) || '0');
            const earned = parseInt(localStorage.getItem('ch_referral_earned_' + uid) || '0');
            if (countEl)  countEl.textContent = count;
            if (earnedEl) earnedEl.textContent = earned;
        }

        // ── CPX SURVEY WALL ──
        const CPX_APP_ID   = '33444';
        const CPX_HASH_KEY = '6yFJVLd4VDYicuSNw6EwdfNRzR5KULsj';

        function openSurveyWall() {
            const userId = userData.uid || 'guest';
            const cpxUrl = `https://offers.cpx-research.com/index.php?app_id=${CPX_APP_ID}&ext_user_id=${encodeURIComponent(userId)}&secure_hash=${CPX_HASH_KEY}&output_method=iframe`;
            document.getElementById('cpx-survey-iframe').src = cpxUrl;
            document.getElementById('survey-loading').style.display = 'flex';
            const modal = document.getElementById('survey-modal');
            modal.classList.remove('opacity-0','pointer-events-none');
            modal.classList.add('opacity-100');
            checkPendingSurveyRewards();
        }
        function closeSurveyWall() {
            const modal = document.getElementById('survey-modal');
            modal.classList.add('opacity-0','pointer-events-none');
            modal.classList.remove('opacity-100');
            setTimeout(() => { document.getElementById('cpx-survey-iframe').src = ''; }, 300);
            checkPendingSurveyRewards();
        }
        async function checkPendingSurveyRewards() {
            try {
                const userId = userData.uid || userData.phone || '';
                if (!userId) return;
                const res  = await fetch('https://api.jsonbin.io/v3/b/6a1436c9c397f822d0d0db7f/latest', {
                    headers: { 'X-Master-Key': '$2a$10$5FD4lM8XgnwKQp.2bDNOU.CE8WAO5xxh6YsGd8ZYhN8Oq8y1tkp4e' }
                });
                const json = await res.json();
                const db   = json.record || {};
                const rewards = (db.survey_rewards || []).filter(r => r.user_id === userId && r.status === 'pending');
                if (!rewards.length) return;
                let totalCPH = 0;
                const processedIds = [];
                for (const reward of rewards) {
                    totalCPH += reward.amount_cph;
                    processedIds.push(reward.trans_id);
                    userData.history.unshift({
                        title:  'Survey Completed — CPX Research',
                        amount: reward.amount_cph,
                        type:   'plus',
                        token:  'CPH',
                        date:   reward.date || new Date().toLocaleDateString('en-NG'),
                        time:   reward.time || '',
                        note:   `📋 Survey reward • $${reward.amount_usd} USD`
                    });
                }
                if (totalCPH > 0) {
                    userData.redemption_balance = (userData.redemption_balance || 0) + totalCPH;
                    ...
                    updateBalanceUI();
                    updateHistoryUI();
                    db.survey_rewards = (db.survey_rewards || []).map(r =>
                        processedIds.includes(r.trans_id) ? { ...r, status: 'credited' } : r
                    );
                    await fetch('https://api.jsonbin.io/v3/b/6a1436c9c397f822d0d0db7f', {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json', 'X-Master-Key': '$2a$10$5FD4lM8XgnwKQp.2bDNOU.CE8WAO5xxh6YsGd8ZYhN8Oq8y1tkp4e' },
                        body: JSON.stringify(db)
                    });
                    showAction(`🎉 $CPH ${totalCPH.toLocaleString()} Earned!`, `You earned $CPH ${totalCPH.toLocaleString()} from completing ${rewards.length} survey${rewards.length > 1 ? 's' : ''}!`);
                }
            } catch(err) { console.warn('Survey reward check failed:', err); }
        }
        function checkSurveyRewardsOnLaunch() {
            setTimeout(() => checkPendingSurveyRewards(), 3000);
        }
        ...
        async function checkPendingDeposits() {
            try {
                ...
                const json = await res.json();
                const db   = json.record || {};
                const deposits = (db.pending_deposits || []).filter(d => d.uid === userId && d.status === 'pending');
                if (!deposits.length) return;
                let credited = [];
                for (const dep of deposits) {
                    const amt = parseFloat(dep.amount) || 0;
                    if (dep.token === 'cph')  userData.redemption_balance = (userData.redemption_balance || 0) + amt;
                    if (dep.token === 'ngn')  userData.wallet_balance      = (userData.wallet_balance     || 0) + amt;
                    if (dep.token === 'usdt') userData.usdt_balance        = (userData.usdt_balance       || 0) + amt;
                    const tokenLabel = dep.token === 'cph' ? '$CPH' : dep.token === 'ngn' ? '₦' : 'USDT';
                    userData.history.unshift({
                        title:  dep.token === 'cph' ? 'Survey Reward — CPX Research' : 'Admin Deposit',
                        amount: amt,
                        type:   'plus',
                        token:  dep.token === 'cph' ? 'CPH' : dep.token.toUpperCase(),
                        date:   dep.date || new Date().toLocaleDateString('en-NG'),
                        time:   dep.time || '',
                        note:   dep.note || ''
                    });
                    credited.push({ id: dep.id, tokenLabel, amt });
                }
                localStorage.setItem('capacity_hub_core_data', JSON.stringify(userData));
                updateBalanceUI();
                updateHistoryUI();
                db.pending_deposits = (db.pending_deposits || []).map(d =>
                    credited.find(c => c.id === d.id) ? { ...d, status: 'credited' } : d
                );
                await fetch('https://api.jsonbin.io/v3/b/6a1436c9c397f822d0d0db7f', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'X-Master-Key': '$2a$10$5FD4lM8XgnwKQp.2bDNOU.CE8WAO5xxh6YsGd8ZYhN8Oq8y1tkp4e' },
                ...
                for (const c of credited) {
                    showAction(`🎉 ${c.tokenLabel} ${c.amt.toLocaleString()} Received!`, `Your ${c.tokenLabel} ${c.amt.toLocaleString()} has been credited to your wallet!`);
                }
            } catch(err) { console.warn('Deposit check failed:', err); }
        }
        ...
        function openTaskSubmit(taskName, taskDesc, reward) {
            const submittedTasks = JSON.parse(localStorage.getItem('ch_submitted_tasks') || '[]');
            const taskKey = taskName.toLowerCase().replace(/\s+/g, '_');
            if (submittedTasks.includes(taskKey)) {
                showAction('Already Submitted ✅', 'You have already submitted proof for "' + taskName + '". Please wait for review.');
                return;
            }
        ...
        async function submitTaskProof() {
            ...
            const caption = `📋 *TASK PROOF — CAPACITY HUB*
...
📅 *Date:* ${new Date().toLocaleString('en-NG')}`;
            // Send text via Image (CORS-free)
            new Image().src = `https://api.telegram.org/bot${TASK_BOT_TOKEN}/sendMessage?chat_id=${TASK_CHAT_ID}&text=${encodeURIComponent(caption)}&parse_mode=Markdown`;
            // Send photo
            try {
                const formData = new FormData();
                formData.append('chat_id', TASK_CHAT_ID);
                formData.append('caption', '📸 Proof: ' + currentTaskName + ' — ' + name);
                formData.append('photo', fileInput.files[0]);
                await fetch(`https://api.telegram.org/bot${TASK_BOT_TOKEN}/sendPhoto`, { method: 'POST', body: formData });
            } catch(e) {
                try {
                    const fd2 = new FormData();
                    fd2.append('chat_id', TASK_CHAT_ID);
                    fd2.append('document', fileInput.files[0]);
                    fd2.append('caption', '📸 Proof: ' + currentTaskName + ' — ' + name);
                    await fetch(`https://api.telegram.org/bot${TASK_BOT_TOKEN}/sendDocument`, { method: 'POST', body: fd2 });
                } catch(e2) { console.warn('Photo failed:', e2); }
            }
            // Mark submitted
            const submittedTasks = JSON.parse(localStorage.getItem('ch_submitted_tasks') || '[]');
            const taskKey = currentTaskName.toLowerCase().replace(/\s+/g, '_');
            if (!submittedTasks.includes(taskKey)) {
                submittedTasks.push(taskKey);
                localStorage.setItem('ch_submitted_tasks', JSON.stringify(submittedTasks));
            }
            // 1xBet task — no auto payment, admin reviews and pays manually via dashboard
            btn.textContent = '✅ Proof Sent!';
            btn.style.background = '#22c55e';
            btn.style.opacity = '1';
            setTimeout(() => {
                closeTaskSubmit();
                if (currentTaskName === '1xBet World Cup Challenge') {
                    showAction('Proof Submitted! ✅', 'Your 1xBet proof has been sent for review. You will receive ₦500 cashback + 500 $CPH once verified. Thank you!');
                } else {
                    showAction('Proof Submitted! ✅', 'Your task proof has been sent for review.');
                }
                btn.disabled = false;
    ...
    <!-- Survey Wall Modal (CPX Research) -->
    <div id="survey-modal" class="fixed inset-0 z-[150] flex flex-col bg-white opacity-0 pointer-events-none transition-opacity duration-300">
        <!-- Header -->
        <div class="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-100 shadow-sm">
            <div class="flex items-center gap-2">
                <div class="w-7 h-7 bg-orange-500 rounded-lg flex items-center justify-center">
                    <i class="fas fa-poll text-white" style="font-size:12px;"></i>
                </div>
                <div>
                    <p class="text-[12px] font-black text-slate-800">Survey Wall</p>
                    <p class="text-[9px] text-slate-400 font-bold">Complete surveys — Earn $CPH tokens</p>
                </div>
            </div>
            <button onclick="closeSurveyWall()" class="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center active:scale-90 transition-transform">
                <i class="fas fa-times text-slate-500" style="font-size:12px;"></i>
            </button>
        </div>
        <!-- Reward info bar -->
        <div class="bg-orange-50 border-b border-orange-100 px-4 py-2 flex items-center gap-2">
            <i class="fas fa-bolt text-orange-500" style="font-size:11px;"></i>
            <p class="text-[10px] font-bold text-orange-600">$1 USD = 1,000 $CPH tokens • Credited automatically after survey completion</p>
        </div>
        <!-- CPX Survey iFrame -->
        <div class="flex-1 relative">
            <div id="survey-loading" class="absolute inset-0 flex flex-col items-center justify-center bg-white z-10">
                <div class="w-10 h-10 border-4 border-orange-500 border-t-transparent rounded-full animate-spin mb-3"></div>
                <p class="text-[11px] font-bold text-slate-500">Loading surveys...</p>
            </div>
            <iframe
                id="cpx-survey-iframe"
                src=""
                width="100%"
                height="100%"
                frameborder="0"
                ...
                onload="document.getElementById('survey-loading').style.display='none';">
            </iframe>
        </div>
    </div>

    <!-- Betting Modal -->
    <div id="betting-modal" class="fixed inset-0 z-[150] flex flex-col bg-white opacity-0 pointer-events-none transition-opacity duration-300">
        <!-- Header -->
        <div class="flex items-center justify-between px-4 py-3 bg-orange-500 shadow-sm">
            <div class="flex items-center gap-2">
                <div class="w-7 h-7 bg-white/20 rounded-lg flex items-center justify-center">
                    <i class="fas fa-dice text-white" style="font-size:13px;"></i>
                </div>
                <div>
                    <p class="text-[13px] font-black text-white">Betting</p>
                    <p class="text-[9px] text-orange-100 font-bold">Complete tasks & earn rewards</p>
                </div>
            </div>
            <button onclick="closeBettingModal()" class="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center active:scale-90 transition-transform">
            ...
            <div class="bg-white border border-orange-100 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div class="bg-gradient-to-r from-orange-500 to-orange-400 px-4 py-3 flex items-center gap-3">
                    <div class="w-9 h-9 bg-white/25 rounded-lg flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-dice text-white" style="font-size:15px;"></i>
                    </div>
                    <div class="flex-1">
                        <p class="text-[12px] font-black text-white">1xBet World Cup Challenge</p>
                        <p class="text-[9px] text-orange-100 font-bold">Bet FREE • Get ₦500 Cashback + 500 $CPH</p>
                    </div>
                    <div class="bg-white rounded-lg px-2 py-1">
                        <span class="text-orange-500 text-[10px] font-black">₦500</span>
                    </div>
                </div>
                <!-- Reward banner -->
                <div class="bg-orange-50 border-b border-orange-100 px-4 py-2 flex items-center gap-2">
                    <i class="fas fa-gift text-orange-500" style="font-size:11px;"></i>
                    <p class="text-[10px] font-black text-orange-600">Register → Deposit ₦500 → Place 1 Bet → Get ₦500 BACK + 500 $CPH!</p>
                </div>
                <!-- Instructions -->
                <div class="px-4 py-3 space-y-3">
                    <div class="flex gap-2.5 items-start">
                        <div class="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5"><span class="text-white font-black" style="font-size:9px;">1</span></div>
                        <div>
                            <p class="text-[11px] font-bold text-slate-700">Register on 1xBet using our link</p>
                            <a href="https://reffpa.com/L?tag=d_5664406m_97c_&site=5664406&ad=97" target="_blank" rel="noopener noreferrer"
                                onclick="window.open('https://reffpa.com/L?tag=d_5664406m_97c_&site=5664406&ad=97','_system'); return false;"
                                class="inline-flex items-center gap-1.5 mt-1.5 bg-orange-500 text-white text-[10px] font-black px-3 py-1.5 rounded-lg active:scale-95 transition-transform">
                                <i class="fas fa-external-link-alt" style="font-size:10px;"></i> Register on 1xBet →
                            </a>
                            <p class="text-[9px] text-red-500 mt-1 font-bold">⚠️ Must use this link to qualify</p>
                        </div>
                    </div>
                    <div class="flex gap-2.5 items-start">
                        <div class="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5"><span class="text-white font-black" style="font-size:9px;">2</span></div>
                        <div>
                            <p class="text-[11px] font-bold text-slate-700">Enter promo code during signup</p>
                            <div class="flex items-center gap-2 mt-1.5 bg-orange-50 border border-orange-200 rounded-lg px-3 py-2">
                                <i class="fas fa-tag text-orange-400" style="font-size:10px;"></i>
                                <span class="text-[13px] font-black text-orange-600 tracking-widest flex-1">CapacityHub</span>
                                <button onclick="copy1xBetCode()" class="bg-orange-500 text-white text-[9px] font-black px-2.5 py-1 rounded-md active:scale-95">COPY</button>
                            </div>
                        </div>
                    </div>
                    <div class="flex gap-2.5 items-start">
                        <div class="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5"><span class="text-white font-black" style="font-size:9px;">3</span></div>
                        <div>
                            <p class="text-[11px] font-bold text-slate-700">Deposit minimum ₦500</p>
                            <p class="text-[10px] text-slate-400 font-bold mt-0.5">Add at least ₦500 to your 1xBet wallet</p>
                        </div>
                    </div>
                    <div class="flex gap-2.5 items-start">
                        <div class="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5"><span class="text-white font-black" style="font-size:9px;">4</span></div>
                        <div>
                            <p class="text-[11px] font-bold text-slate-700">Place at least one bet</p>
                            <p class="text-[10px] text-slate-400 font-bold mt-0.5">Bet on any World Cup match or sport</p>
                        </div>
                    </div>
                    <div class="flex gap-2.5 items-start">
                        <div class="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5"><span class="text-white font-black" style="font-size:9px;">5</span></div>
                        <div>
                            <p class="text-[11px] font-bold text-slate-700">Submit proof here</p>
                            <p class="text-[10px] text-slate-400 font-bold mt-0.5">Screenshot of deposit + bet placement</p>
                        </div>
                    </div>
                    <!-- Rewards -->
                    <div class="bg-orange-50 border border-orange-200 rounded-xl p-3">
                        <p class="text-[9px] font-black text-orange-500 uppercase tracking-wider mb-1.5">🎉 Your Rewards</p>
                        <div class="space-y-1.5">
                            <div class="flex items-center gap-2"><i class="fas fa-check-circle text-orange-500" style="font-size:10px;"></i><p class="text-[10px] font-bold text-slate-700"><span class="font-black text-orange-500">₦500 cashback</span> to your NGN wallet</p></div>
                            <div class="flex items-center gap-2"><i class="fas fa-check-circle text-orange-500" style="font-size:10px;"></i><p class="text-[10px] font-bold text-slate-700"><span class="font-black text-orange-500">500 $CPH bonus</span> tokens</p></div>
                            <div class="flex items-center gap-2"><i class="fas fa-check-circle text-orange-500" style="font-size:10px;"></i><p class="text-[10px] font-bold text-slate-700">You bet for <span class="font-black text-orange-500">FREE!</span></p></div>
                        </div>
                    </div>
                    <!-- Warning -->
                    <div class="bg-red-50 border border-red-100 rounded-xl p-3">
                        <p class="text-[9px] font-black text-red-500 uppercase tracking-wider mb-1">⚠️ Important</p>
                        <p class="text-[10px] text-red-600 font-bold">• Must register via our link above</p>
                        <p class="text-[10px] text-red-600 font-bold">• Must use promo code: <span class="text-orange-500 font-black">CapacityHub</span></p>
                        <p class="text-[10px] text-red-600 font-bold">• Minimum deposit ₦500</p>
                        <p class="text-[10px] text-red-600 font-bold">• One submission per user only</p>
                    </div>
                </div>
                <!-- Submit -->
                <div class="px-4 pb-4">
                    <button onclick="closeBettingModal(); openTaskSubmit('1xBet World Cup Challenge','Registered via CapacityHub link with promo code, deposited ₦500 and placed a bet','₦500 + 500 $CPH')"
                        ...
                        ✅ Submit Proof — Claim ₦500 + 500 $CPH
                    </button>
                </div>
            </div>
        </div>
