...
<body class="pb-24">
    ...
    <!-- Supabase SDK — pinned stable version -->
    ...
    <script>
        ...
        // SUPABASE CONFIG
        // ══════════════════════════════════════════
        const SUPA_URL = 'https://bjwitwiowsbgddfdcgjh.supabase.co';
        const SUPA_KEY = 'sb_publishable_XWitvEM2jpIabwMzxUAKzA_lBkrP0EG';
        let supa = null;
        ...
        // ── Init Supabase safely — called after splash ──
        function initSupabase() {
            // Try all known UMD export names
            const client =
                (window.supabase && typeof window.supabase.createClient === 'function') ? window.supabase.createClient :
                (window.Supabase && typeof window.Supabase.createClient === 'function') ? window.Supabase.createClient :
                null;
            ...
            try {
                supa = client(SUPA_URL, SUPA_KEY);
                console.log('✅ Supabase ready');
                // Load tasks/rate in background — non-blocking
                sbLoadAppData();
                startAppMetaListener();
        ...
        async function sbSaveUser() {
            ...
            try {
                ...
                const { error } = await supa.from('users').upsert(p, { onConflict: 'uid' });
                if (error) console.error('❌ sbSaveUser:', error.message);
                else console.log('✅ User saved:', userData.uid);
        ...
        async function sbLoadUser(uid) {
            if (!supa) return null;
            try {
                const { data, error } = await supa.from('users').select('*').eq('uid', uid).single();
        ...
        async function sbPhoneExists(phone) {
            ...
            try {
                const { data } = await supa.from('users').select('uid').eq('phone', phone);
        ...
        async function sbLogin(name, password) {
            ...
            try {
                const { data } = await supa.from('users').select('*')
                    .ilike('name', name).eq('password', password).limit(1);
        ...
        async function sbLoadAppData() {
            ...
            try {
                ...
                if (data && data.payload) {
                    ...
                    localStorage.setItem('ch_admin_db', JSON.stringify(Object.assign(local, data.payload)));
                }
        ...
        async function sbSaveSubmission(sub) {
            if (!supa) return;
            try {
                const { error } = await supa.from('submissions').upsert(sub, { onConflict: 'id' });
                if (error) console.error('❌ sbSaveSubmission:', error.message);
                else console.log('✅ Submission saved');
        ...
        async function sbSaveWithdrawal(wd) {
            if (!supa) return;
            try {
                const { error } = await supa.from('withdrawals').upsert(wd, { onConflict: 'id' });
                if (error) console.error('❌ sbSaveWithdrawal:', error.message);
                else console.log('✅ Withdrawal saved');
            } catch(e) { console.error('❌ sbSaveWithdrawal:', e.message); }
        }

        function startUserListener() {
            if (!supa || !userData.uid) return;
            supa.channel('user-' + userData.uid)
                .on('postgres_changes', {
                    ...
                    const d = payload.new;
                    if (!d) return;
                    const changed = d.wallet_balance !== userData.wallet_balance
                        || d.usdt_balance !== userData.usdt_balance
                        || (d.history||[]).length > (userData.history||[]).length;
                    if (changed) {
                        userData.wallet_balance = d.wallet_balance||0;
                        userData.usdt_balance = d.usdt_balance||0;
                        userData.redemption_balance = d.redemption_balance||0;
                        userData.history = d.history||[];
        ...
        function startAppMetaListener() {
            if (!supa) return;
            supa.channel('meta-updates')
                .on('postgres_changes', {
                    event: '*', schema: 'public', table: 'app_meta', filter: 'id=eq.main'
                }, payload => {
                    if (payload.new && payload.new.payload) {
                        const local = loadAdminDB() || {};
                        localStorage.setItem('ch_admin_db', JSON.stringify(Object.assign(local, payload.new.payload)));
        ...
        function syncUserToAdminDB() { if (supa) sbSaveUser(); }
        ...
        function showNextScreen() {
            try {
                ...
                if (parsed && parsed.loggedIn) {
                    ...
                    // Refresh from Supabase in background — retries until ready
                    const doRefresh = () => {
                        if (!supa) { setTimeout(doRefresh, 600); return; }
