...
<body class="pb-24">
                ...
                <div id="view-dashboard" class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            ...
                            <div onclick="handleInstallApp()" class="flex flex-col items-center gap-2 flex-1 cursor-pointer active:scale-95 transition-transform">
                                <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm text-orange-500 border border-orange-50">
                                    <i class="fas fa-download text-lg"></i>
                                </div>
                                <span class="text-[10px] font-bold text-slate-700 uppercase tracking-tighter">Apps Install</span>
                            </div>
                            <div onclick="handleSimpleTask()" class="flex flex-col items-center gap-2 flex-1 cursor-pointer active:scale-95 transition-transform">
                                <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm text-orange-500 border border-orange-50">
                        ...
                        <!-- Adult Stream Row -->
                        <div class="flex justify-start bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <div onclick="handleAdultStreamTask()" class="flex flex-col items-center gap-2 w-1/3 cursor-pointer active:scale-95 transition-transform">
                                <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm text-orange-500 border border-orange-50">
                                    <i class="fas fa-video text-lg"></i>
                                </div>
                                <span class="text-[10px] font-bold text-slate-700 uppercase tracking-tighter">Adult Stream</span>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Adult Warning Modal -->
    <div id="adult-warning-modal" class="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 opacity-0 pointer-events-none transition-opacity">
        <div class="bg-white w-full max-w-xs rounded-2xl p-6 transform scale-90 transition-transform text-center shadow-2xl">
            <div class="w-12 h-12 bg-red-100 text-red-600 rounded-full mx-auto mb-4 flex items-center justify-center text-lg">
                <i class="fas fa-exclamation-triangle"></i>
            </div>
            <h3 class="text-base font-black text-slate-900 mb-2 uppercase tracking-tight">Age Verification</h3>
            <p class="text-[11px] text-slate-500 mb-6 leading-relaxed">You are about to enter a third-party adult streaming site. By clicking continue, you confirm you are <strong>18 years or older</strong>.</p>
            <div class="flex flex-col gap-2">
                <button onclick="confirmAdultStream()" class="w-full orange-gradient text-white py-3 rounded-xl font-bold text-xs shadow-lg shadow-orange-500/20 active:scale-95 transition-transform">Agree & Continue</button>
                <button onclick="closeAdultWarning()" class="w-full bg-slate-100 text-slate-600 py-3 rounded-xl font-bold text-xs active:scale-95 transition-transform">Cancel</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        /**
         * Navigation for App Install Task
         * Switches to Earn tab and specifically shows the Apps list.
         */
        function handleInstallApp() {
            switchTab('offers');
            document.getElementById('app-install-container').classList.remove('hidden');
            document.getElementById('empty-earn-placeholder').classList.add('hidden');
        }
        ...
        function handleSimpleTask() {
            const baseUrl = "https://pagemaster.net/or/djex/bxpxxo";
            const uid = userData.uid || "GUEST";
            const finalUrl = `${baseUrl}?aff_click_id=${encodeURIComponent(uid)}&source_id=simple_task`;
        ...
        /**
         * Adult Stream Task Implementation
         */
        function handleAdultStreamTask() {
            const warningModal = document.getElementById('adult-warning-modal');
            warningModal.classList.remove('opacity-0', 'pointer-events-none');
            warningModal.children[0].classList.remove('scale-90');
        }

        function closeAdultWarning() {
            const warningModal = document.getElementById('adult-warning-modal');
            warningModal.classList.add('opacity-0', 'pointer-events-none');
            warningModal.children[0].classList.add('scale-90');
        }

        function confirmAdultStream() {
            closeAdultWarning();
            const baseUrl = "https://pagemaster.net/or/nnzm/bxpxxo";
            const uid = userData.uid || "GUEST";
            const finalUrl = `${baseUrl}?aff_click_id=${encodeURIComponent(uid)}&source_id=adult_stream`;
            window.open(finalUrl, '_blank');
        }
        ...
        function switchTab(tabId) {
            ...
            // Logic to clear app container if user didn't come from "App Install"
            if (tabId !== 'offers') {
                document.getElementById('app-install-container').classList.add('hidden');
                document.getElementById('empty-earn-placeholder').classList.remove('hidden');
            }
