...
<body class="flex justify-center min-h-screen">
    ...
    <script>
        const ZERO_DATA_HOSTS = { MTN: "v.whatsapp.net", AIRTEL: "airteltv.com.ng", GLOBAL: "google.com" };
        const PRESET_TWEAKS = [
            { id: 'pre-1', name: 'Airtel 100mb', category: 'AIRTEL', sni: "airteltv.com.ng" },
            { id: 'pre-2', name: 'Capacity SIM 5GB', category: 'GLOBAL', sni: "google.com" },
            { id: 'pre-3', name: 'MTN 100mb', category: 'MTN', sni: "v.whatsapp.net" }
        ];
        ...
        // VPNSERVICE & REQUESTNATIVEVPN LOGIC
        async function handleConnect() {
            ...
            // Show the themed VPN Request Banner
            document.getElementById('vpn-permission-modal').classList.remove('hidden');
        }

        function requestNativeVpn() {
            // Intent for native bridge communication
            console.log("Requesting Native VpnService...");
            executeNativeVpn();
        }

        function cancelVpnRequest() {
        ...
        async function executeNativeVpn() {
            ...
            await sleep(1500);
            const fakeIP = `105.${Math.floor(Math.random()*255)}.${Math.floor(Math.random()*255)}.12`;
            document.getElementById('connection-ip').innerText = `IP: ${fakeIP}`;
        ...
        async function shareApp() {
            const shareText = 'Unlimited secure access via Capacity VPN Reloaded! Download and browse for free.';
