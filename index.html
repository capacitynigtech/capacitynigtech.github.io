...
<body class="pb-24">
    ...
    <script>
        // ── SHARED DB HELPERS (connects to Dashboard via ch_admin_db) ──
        ...
        window.addEventListener('load', () => {
            // ── Load or create userData, then sync into ch_admin_db ──
            const savedData = localStorage.getItem('capacity_hub_core_data');
            if (savedData) {
                userData = JSON.parse(savedData);
                if (userData.usdt_balance === undefined) userData.usdt_balance = 0.00;
                // Migrate: if uid is old UID-format and phone exists, use phone as uid
                if (userData.phone && userData.uid && userData.uid.startsWith('UID-')) {
                    userData.uid = userData.phone;
                }
            } else {
                userData.uid = `TEMP-${Math.floor(10000000 + Math.random() * 90000000)}`;
                userData.usdt_balance = 0.00;
                saveToPhone();
            }

            // ── Register user in Dashboard DB if not already there ──
            syncUserToAdminDB();

            updateBalanceUI();
            updateHistoryUI();

            // ── Render tasks from Dashboard ──
            ...
            // ── Poll every 3s for dashboard-pushed rewards ──
        ...
        function signUp() {
            ...
            // Check if phone already registered in admin DB
            ...
            // UID = phone number only (without country code)
            userData = {
                uid: phone,
                name: name,
                phone: fullPhone,
        ...
        function logIn() {
            ...
            // Also check local storage (in case user only registered on device)
            if (!matchedUser) {
            ...
            // Load matched user into userData
            userData = {
        ...
        function syncUserToAdminDB() {
            ...
            if (!existing) {
                ...
                // Push latest balances up to dashboard
        ...
        function pollDashboardUpdates() {
            ...
            // Detect if admin pushed new balance/history (approval)
            const dbHistLen = (dbUser.history || []).length;
            if (dbHistLen > lastHistoryLength) {
                // Pull updated data from dashboard into local userData
                ...
                userData.history = dbUser.history || [];
                lastHistoryLength = dbHistLen;
                saveToPhone();
                updateBalanceUI();
                updateHistoryUI();
                // Show notification dot for new reward
        ...
        function openSubmitProofModal(taskId) {
            ...
            // Build a lightweight inline modal dynamically
            ...
            div.innerHTML = `
                <div class="bg-white w-full max-w-xs rounded-2xl p-6 shadow-2xl">
                    ...
                    <input type="text" id="proof-screenshot" class="w-full p-3 border border-slate-200 rounded-xl text-[11px] outline-none mb-4" placeholder="Screenshot URL (optional)">
                    <div class="flex gap-2">
        ...
        function submitTaskProof(taskId) {
            ...
            const screenshot = document.getElementById('proof-screenshot').value.trim();
            if (!note) { alert('Please describe what you completed.'); return; }

            const db = loadAdminDB();
            if (!db) return;
            if (!db.submissions) db.submissions = [];

            // Prevent duplicate pending submission for same task
            const already = db.submissions.find(s => s.userId === userData.uid && s.taskId === taskId && s.status === 'pending');
            if (already) { alert('You already have a pending submission for this task.'); document.getElementById('proof-modal')?.remove(); return; }
        ...
        function processWithdrawal() {
            ...
            if (targetField === 'usdt_balance' || targetField === 'wallet_balance') {
                ...
                setTimeout(() => {
                    ...
                    addTransaction(`Withdrawal (${title})`, amount, 'minus');
                    updateBalanceUI();
                    document.getElementById('withdraw-processing').classList.add('hidden');
        ...
        function setSwapDirection(dir) {
            ...
            if (dir === 'usdt-ngn') {
                ...
                document.getElementById('swap-rate-label').innerText = 'Rate: 1 USDT = ₦1,500';
            } else {
                ...
                document.getElementById('swap-rate-label').innerText = 'Rate: ₦1,500 = 1 USDT';
            }
            updateSwapPreview();
        }
        ...
        function updateSwapPreview() {
            ...
            if (swapDirection === 'usdt-ngn') {
                preview.innerText = `You will receive: ₦${(amount * SWAP_RATE).toLocaleString('en-NG', {minimumFractionDigits: 2})}`;
            } else {
                preview.innerText = `You will receive: ${(amount / SWAP_RATE).toFixed(4)} USDT`;
            }
        }
        ...
        function processSwap() {
            ...
            if (swapDirection === 'usdt-ngn') {
                ...
                const ngnReceived = amount * SWAP_RATE;
                ...
                const usdtReceived = amount / SWAP_RATE;
                document.getElementById('swap-form').classList.add('hidden');
                document.getElementById('swap-processing').classList.remove('hidden');
                setTimeout(() => {
                    userData.wallet_balance -= amount;
